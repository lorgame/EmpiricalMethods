  ...
  <div class="containerPage">
    ...
    <div class="containerTask">
      ...
      <div class="blockNumber">
        ...
        <div v-show="arrayResultChange.length !== 0" class="tableChangeNumber">
          ...
          <div class="blockVertical">
            <div class="tableTitleVertical">
              <span class="cellTitle" v-for="(value, i) of arrayTask" :key="i">{{value}}</span>
            </div>
            <div class="tableValue">
              <div class="cell" v-for="(value, i) of 3" :key="i">{{numberForChange}}</div>
            </div>
            <div class="tableValue">
              <div class="cell" v-for="(value, i) of arrayResultChange" :key="i">{{value}}</div>
            </div>
            <div class="tableValue">
              <div class="cell" v-for="(value, i) of 3" :key="i"><button @click="viewChange(i)" class="createButton">Згенерувати</button></div>
            </div>
          </div>
        </div>
        <div class="blockNumber">
...
export default {
  ...
  methods: {
    ...
    changeNumber () {
      if (parseInt(this.n) !== 0 && this.arrayNormalNumbers.length !== 0 && this.typeNumber !== '') {
        this.arrayResultChange.length = 0
        let array = []
        ...
        for (let i = 0; i < array.length; i++) {
          middleNumber += Number(array[i])
        }
        let sumNumber = 0
        for (let i = 0; i < array.length; i++) {
          sumNumber += Math.pow(Number(array[i]) - middleNumber, 2)
        }
        const middleArithmeticNumber = Math.sqrt(sumNumber / array.length)
        const firstChangeNumber = middleArithmeticNumber * 0.3
        const secondChangeNumber = middleArithmeticNumber * 1
        const thirdChangeNumber = middleArithmeticNumber * 15
        this.arrayResultChange.push(firstChangeNumber, secondChangeNumber, thirdChangeNumber)
    ...
    viewChange (i) {
      if (this.typeNumber !== '') {
        ...
        if (this.typeNumber === 'Найбільше') {
          ...
          if (i === 0) {
            this.arrayChangeNumber[arrayChangeNumberLength - 1] = this.arrayResultChange[0]
          } else if (i === 1) {
            this.arrayChangeNumber[arrayChangeNumberLength - 1] = this.arrayResultChange[1]
          } else if (i === 2) {
            this.arrayChangeNumber[arrayChangeNumberLength - 1] = this.arrayResultChange[2]
          }
        } else if (this.typeNumber === 'Найменше') {
          this.indexItemArray = 0
          if (i === 0) {
            this.arrayChangeNumber[0] = this.arrayResultChange[0]
          } else if (i === 1) {
            this.arrayChangeNumber[0] = this.arrayResultChange[1]
          } else if (i === 2) {
            this.arrayChangeNumber[0] = this.arrayResultChange[2]
          }
        }
      } else {
        alert('Оберіть граничне значення та збережіть його')
    ...
    downloadInfo () {
      ...
      arrayText.push(this.arrayTask[this.indexTaskArray] + '\nВідібране значення: "' + this.numberForChange + '"\nРезультат: "' + this.arrayResultChange[this.indexTaskArray] + '"\nГенерація: \n' + numberOfArrayChange)
...
<style scoped lang="scss">
  .containerPage {
    ...
    .blockNumberStudent, .blockTypeNumber, .blockSaveInfo, .blockSaveError {
      display: flex;
      margin: 5px 0 15px 0;
    }
    .blockNumberStudent {
  ...
  @media (min-width: 0px) and (max-width: 1050px) {
    .containerPage {
      .blockNumberStudent, .blockTypeNumber, .blockSaveInfo, .blockSaveError, .containerTask {
        justify-content: center;
      }
    }
  }
  @media (min-width: 0px) and (max-width: 768px) {
    .containerPage {
      ...
      .blockNumberStudent, .blockSaveInfo, .blockSaveError, .blockTypeNumber {
